    ...
    fn from_op_tx(tx: &OpTxEnvelope) -> Result<Self, SpanBatchError> {
        let parts = match tx {
            OpTxEnvelope::Legacy(signed) => {
                let (tx, sig) = (signed.tx(), *signed.signature());
                Self {
                    data: SpanBatchTransactionData::try_from(&TxEnvelope::Legacy(signed.clone()))?,
                    signature: sig,
                    to: tx.to(),
                    nonce: tx.nonce(),
                    gas: tx.gas_limit(),
            ...
            OpTxEnvelope::Eip2930(signed) => {
                let (tx, sig) = (signed.tx(), *signed.signature());
                Self {
                    data: SpanBatchTransactionData::try_from(&TxEnvelope::Eip2930(signed.clone()))?,
                    signature: sig,
                    to: tx.to(),
                    nonce: tx.nonce(),
                    gas: tx.gas_limit(),
            ...
            OpTxEnvelope::Eip1559(signed) => {
                let (tx, sig) = (signed.tx(), *signed.signature());
                Self {
                    data: SpanBatchTransactionData::try_from(&TxEnvelope::Eip1559(signed.clone()))?,
                    signature: sig,
                    to: tx.to(),
                    nonce: tx.nonce(),
                    gas: tx.gas_limit(),
            ...
            OpTxEnvelope::Eip7702(signed) => {
                let (tx, sig) = (signed.tx(), *signed.signature());
                Self {
                    data: SpanBatchTransactionData::try_from(&TxEnvelope::Eip7702(signed.clone()))?,
                    signature: sig,
                    to: tx.to(),
                    nonce: tx.nonce(),
                    gas: tx.gas_limit(),
            ...
            OpTxEnvelope::PostExec(tx) => Self {
                data: SpanBatchTransactionData::PostExec(SpanBatchPostExecTransactionData {
