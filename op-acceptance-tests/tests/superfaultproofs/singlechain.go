...
func prepareSDMSingleChainTarget(t devtest.T, sys *presets.SingleChainInterop, c *chain, _ []*chain) (uint64, uint64, uint64) {
	block, targetBlock := mustFindRepeatedSlotBlock(t, sys, 2, 3)
	postExecTx, postExecIndex := sdmpkg.FindPostExecTransaction(block)
	t.Require().NotNil(postExecTx, "SDM-enabled sequencer must include a post-exec tx")
	t.Require().Equal(len(block.Transactions)-1, postExecIndex, "post-exec tx must be the trailing transaction")
...
func mustFindRepeatedSlotBlock(
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		block := getBlockWithTxs(t, sys.L2ELA, targetBlockNum)
